    ...
    try {
        ...
        const response = await openai.chat.completions.create({
            ...
            messages: [
                ...
                { role: 'user', content: pages.join("\n") + "\n\nUltimo resumo, siga a partir dele: " + lastSummary },
            ]
        });
...
export async function chatWithDocument(
    ...
    RAGFusion(queries, documentId).then(
        async (RAGResult) => {
            const result = RAGResult.map(result => result.content);
            const lastMessage = messages.pop();
            
            lastMessage.content =   JSON.stringify(result) + 
...
function streamDocumentContext(event:IpcMainInvokeEvent, result:RankedSearchResult[]){
    result.map(result => event.sender.send('chat-stream-files', result))
}

async function streamCompletion(event:IpcMainInvokeEvent, messages: IMessage[], channel:string){
    const openai = await OpenAIClient()
...
async function getMoreQueries( question:IMessage, queryNum:number = 5):Promise<string[]>{
    const openai = await OpenAIClient()
