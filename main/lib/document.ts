...
export async function processPDF({event, pages, pdfPath, bookName, collectionId}:HProcessPDF){
    try {

        const storagePdfPath = await savePdfToStorage(pdfPath, bookName);
        const totalPages = pages.length;
        
        const embeddingCost = countTokens(pages.join(" "), Models.embedding.tiktoken) * Models.embedding.pricingPer1M / MillionTokens;
        event.sender.send("embedding_cost", embeddingCost)
        
        await ensureCollectionExists();

        const cover = await extractCover(pdfPath, bookName);
        const document = await saveDocument({name:bookName, path:storagePdfPath, cover, totalPages});
