...
export default function PDFViewer({document, path }: PDFViewerProps) {
  ...
  return (
    <div className='relative'>
        <canvas ref={canvasRef} className="mx-auto mt-8 w-[800px] h-full rounded-lg shadow-neutral-400 shadow-lg" />
        <div className='fixed bg-container p-2 rounded-lg bottom-2 left-[calc(50%+150px)] -translate-x-1/2 flex items-center justify-center gap-x-8'>
          ...
          <span className='text-white'>
            {`Page `}
              <input type="number" min={1} value={currentPage} className='bg-transparent border-none w-12 focus:outline-none focus:border-sky-500 focus:ring-1 focus:ring-sky-500
      [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none text-center underline' onChange={(e) => handlePageChange(e.target.value)}/>
            {` of ${totalPages}`}
          </span>
