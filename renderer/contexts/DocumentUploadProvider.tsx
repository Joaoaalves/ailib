...
const DocumentUploadProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  useEffect(() => {
    ...
    if (window.openai && window.openai.embeddingProgress) {
      window.openai.embeddingProgress(handleProgress, () => router.refresh());
    }

    if (window.openai && window.openai.embeddingCost){
      window.openai.embeddingCost((cost:number) => setEmbeddingCost(cost))
    }
  ...
  return (
    <ProgressContext.Provider value={{ progressPercentage, isEmbedding, embeddingCost }}>
      {children}
...
const useUploadDocument = (): IProgressContext => {
  ...
  if (context === undefined) {
    throw new Error('useUploadDocument must be used within a DocumentUploadProvider');
  }
  return context;
};
